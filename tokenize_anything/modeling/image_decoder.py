...
class ImageDecoder(nn.Module):
    ...
    def get_outputs(self, inputs):
        ...
        query = torch.cat(tokens).unsqueeze(0).expand(prompts_per_batch, -1, -1)
        ...
        # Клонирование токенов для предотвращения in-place модификаций.
        iou_tokens = sam_tokens[0].clone()
        mask_tokens = [token.clone() for token in sam_tokens[1:]]
        
        # Предсказание масок.
        mask_pred = [f(x) for f, x in zip(self.mask_pred, mask_tokens)]
        mask_pred = torch.stack(mask_pred, dim=1) @ mask_embeds
        mask_pred_size = [4 * embed_size for embed_size in img_embed_size]
        mask_pred = mask_pred.view([-1, self.num_mask_tokens] + mask_pred_size)
        
        # Формирование выходного словаря.
        outputs = {
            "iou_pred": self.iou_pred(iou_tokens),
            "mask_pred": mask_pred,
            "sem_tokens": sem_tokens.unsqueeze(2),  # Избегаем использования unsqueeze_
            "sem_embeds": self.sem_pred(sem_tokens.unsqueeze(2).flatten(2))
        }
